...
[ADR 0033](../adr/0033-accept-etcd-raft-core.md) resolves this document's open
consensus choice: etcd/raft v3.7.0 is the accepted core, and the project owns
the durable storage, transport, snapshot transfer, and Multi-Raft host. Sections
that compare Dragonboat remain historical decision input, not the current
selection.
